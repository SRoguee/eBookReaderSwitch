...
struct BookEntry {
    ...
    string extention;
    bool   warned;
    SDL_Texture *cover; // NULL if no cover could be rendered -> placeholder
};
...
void Menu_StartChoosing() {
    ...
    // ---- Build the book list and render cover thumbnails once up front. ----
    fz_context *cctx = fz_new_context(NULL, NULL, FZ_STORE_UNLIMITED);
    fz_register_document_handlers(cctx);
    ...
    for (const auto & entry : fs::directory_iterator(path)) {
        ...
        if (contains(allowedExtentions, extention)) {
            ...
            b.extention  = extention;
            b.warned     = contains(warnedExtentions, extention);
            b.cover      = render_cover(cctx, b.fullpath.c_str());
            books.push_back(b);
        }
    }
